...
|---|---|---|---|
| One-shot strategy | `IAutonStrategy` | `ATagL1Strategy` | one complete autonomous routine that owns the whole loop |
...
Deliberately much simpler — no state, no stepping. `ATagL1Strategy.track(...)` builds one:
...
This *does* own the whole loop — it's the entire autonomous OpMode's body, blocking (via its own `while`) until `driveState` becomes `null`. Notice it actually uses `IState` internally too (for `turretState` and `driveState`, run concurrently by hand inside the same loop) — but the outer shape is "run until done," not "advance one step per external tick." Also notice `shootBalls(...)` in this same file uses plain `ThreadExtensions.TrySleep(2000)` between steps — that's fine here specifically because this strategy already owns the entire loop for the whole autonomous period; blocking doesn't starve anything else, because nothing else is supposed to run during autonomous.
