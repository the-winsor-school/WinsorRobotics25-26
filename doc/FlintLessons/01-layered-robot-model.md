...
Concrete robots vary by *composition*, not by rewriting this flow. `BillyRobot` is built from a `MecanumDrive` and a `BillyMA` (mech assembly). A hypothetical `Wildbots2025` robot could be built from the same `MecanumDrive` but a completely different mech assembly, like `CascadeArm`. The layered shape stays fixed; what you plug into it changes.
